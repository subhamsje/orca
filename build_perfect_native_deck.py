...
# Color Palette - Professional SIH Theme (Light / High Contrast)
CLR_WHITE = RGBColor(255, 255, 255)
CLR_BG_SLATE = RGBColor(248, 250, 252)     # Slate 50
CLR_BG_BLUE = RGBColor(240, 249, 255)      # Light Cyan Blue
CLR_BG_CARD = RGBColor(255, 255, 255)      # Pure White Card
CLR_BG_DARK = RGBColor(10, 25, 47)         # Deep Navy (#0a192f)
CLR_BG_NAVY_CARD = RGBColor(15, 39, 68)    # Navy Card (#0f2744)

CLR_PRIMARY = RGBColor(13, 71, 161)        # Deep SIH Blue (#0d47a1)
CLR_SECONDARY = RGBColor(2, 132, 199)      # Ocean Blue (#0284c7)
CLR_CYAN = RGBColor(6, 182, 212)           # Bright Cyan (#06b6d4)
CLR_EMERALD = RGBColor(16, 185, 129)       # Emerald Safe (#10b981)
CLR_AMBER = RGBColor(217, 119, 6)          # Amber Warning (#d97706)
CLR_ROSE = RGBColor(225, 29, 72)           # Rose Danger (#e11d48)
CLR_PURPLE = RGBColor(124, 58, 237)        # Purple

CLR_TEXT_DARK = RGBColor(15, 23, 42)       # Slate 900
CLR_TEXT_BODY = RGBColor(51, 65, 85)       # Slate 700
CLR_TEXT_MUTED = RGBColor(100, 116, 139)   # Slate 500
CLR_TEXT_WHITE = RGBColor(255, 255, 255)

CLR_BORDER_LIGHT = RGBColor(226, 232, 240) # Slate 200
CLR_BORDER_BLUE = RGBColor(186, 230, 253)  # Sky 200
CLR_BORDER_PRIMARY = RGBColor(13, 71, 161) # Primary Border
...
def create_card(slide, left, top, width, height, bg_color=CLR_WHITE, border_color=CLR_BORDER_LIGHT, border_width=1.0, shape_type=MSO_SHAPE.ROUNDED_RECTANGLE):
...
def setup_slide_header(slide, title_text, badges):
    # Fix and position Title 1 cleanly
    for s in slide.shapes:
        if s.name == "Title 1":
            ...
            s.fill.solid()
            s.fill.fore_color.rgb = CLR_PRIMARY
    ...
    for idx, (b_text, b_col) in enumerate(badges):
        ...
        by = Inches(1.02)
        b_shape, b_tf = create_card(slide, bx, by, badge_w, badge_h, bg_color=b_col, border_color=None)
        p = b_tf.paragraphs[0]
        p.text = b_text
        p.font.name = "Arial"
        p.font.size = Pt(9)
...
def build_deck():
    ...
    for shape in slide1.shapes:
        if shape.name == "TextBox 9":
            ...
            for idx, (label, val) in enumerate(fields):
                ...
                r2.font.bold = False
                r2.font.size = Pt(12)
                r2.font.color.rgb = CLR_TEXT_DARK
        elif shape.name == "Subtitle 3":
            shape.text_frame.text = "TITLE PAGE"

    # Right Hero Card on Slide 1
    _, tf_h1 = create_card(slide1, Inches(7.0), Inches(1.3), Inches(5.8), Inches(5.5),
                           bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.5)
    ...
    s1_cards = [
        ("[SAFETY] Deterministic Safety Shield", "Hard physics capsizing check (Hcrit = 0.6·L) with zero AI hallucination risk", CLR_EMERALD),
        ("[PHYSICS] Parametric Vessel Twin", "Dynamic wave encounter physics tailored to vessel hull length, draft & beam", CLR_CYAN),
        ("[TELECOM] Multi-Hop LoRa Fleet Mesh", "Overcomes 12 NM 4G blackout for zero-cost offshore telemetry up to 50 km", CLR_AMBER)
    ]
    ...
    setup_slide_header(slide2, "IDEA TITLE: ORCA 4.0 — Maritime Decision Intelligence",
                       [("PROPOSED SOLUTION", CLR_CYAN),
                        ("DETAILED EXPLANATION", CLR_PRIMARY),
                        ("HOW IT ADDRESSES PROBLEM", CLR_PRIMARY),
                        ("INNOVATION & UNIQUENESS", CLR_EMERALD)])

    # Top Thesis Callout Box (top: 1.38, height: 0.52)
    _, tf_th = create_card(slide2, Inches(0.6), Inches(1.38), Inches(12.13), Inches(0.52),
                           bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
    ...
    # Left Box: Visual Transformation Pipeline (top: 1.95, width: 7.3, height: 4.25)
    _, tf_t_hdr = create_card(slide2, Inches(0.6), Inches(1.95), Inches(7.3), Inches(4.25),
                              bg_color=CLR_BG_SLATE, border_color=CLR_BORDER_BLUE, border_width=1.0)
    ...
    p_thd.font.color.rgb = CLR_PRIMARY

    # 8 Input Nodes (2 columns of 4 chips on the left)
    inputs_1 = [
        ("WAVES (Hs, Swell Tp, Period)", "Wind sea & swell wave energy spectra"),
        ("WIND (Vel, Gusts, Direction)", "High-res surface boundary wind vectors"),
        ("CYCLONES (IMD Alerts)", "Active storm center coordinates & radius"),
        ("CURRENTS (Surface u,v)", "Ocean surface drift velocity vectors")
    ]
    inputs_2 = [
        ("SATELLITES (SST & Chlor-a)", "Thermal front & pelagic habitat bounds"),
        ("VESSEL (Lwl, Draft, Beam)", "Displacement hull physical profile"),
        ("AIS FEEDS (MMSI Telemetry)", "Real-time vessel position & speed"),
        ("ROUTE (Planned Waypoints)", "Navigational departure & target docks")
    ]

    for idx, (title, sub) in enumerate(inputs_1):
        _, tf_in = create_card(slide2, Inches(0.72), Inches(2.32 + idx * 0.90), Inches(2.0), Inches(0.80),
                               bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
    ...
    for idx, (title, sub) in enumerate(inputs_2):
        _, tf_in = create_card(slide2, Inches(2.80), Inches(2.32 + idx * 0.90), Inches(2.0), Inches(0.80),
                               bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
        p = tf_in.paragraphs[0]
        p.text = title
        p.font.name = "Arial"
    ...
    # Central Engine Box
    _, tf_ce = create_card(slide2, Inches(4.90), Inches(2.32), Inches(2.85), Inches(1.65),
                           bg_color=CLR_SECONDARY, border_color=CLR_CYAN, border_width=1.5)
    p_ce = tf_ce.paragraphs[0]
    p_ce.text = "⚡ ORCA 4.0 ENGINE"
    p_ce.font.name = "Arial"
    p_ce.font.bold = True
    p_ce.font.size = Pt(11)
    p_ce.font.color.rgb = CLR_WHITE
    ...
    p_ce2.font.color.rgb = CLR_WHITE

    # Downstream Decision Boxes
    out_cards = [
        ("DETERMINISTIC RISK", "Hcrit capsizing check (<10ms)", CLR_EMERALD),
        ("ROUTE RISK FIELD", "Dynamic 2D risk surface A*", CLR_CYAN),
        ("ACTIONABLE VERDICT", "Clear Go/No-Go + Audio Voice", CLR_AMBER)
    ]
    for idx, (title, sub, col) in enumerate(out_cards):
        _, tf_oc = create_card(slide2, Inches(4.90), Inches(4.08 + idx * 0.70), Inches(2.85), Inches(0.62),
                               bg_color=CLR_BG_DARK, border_color=col, border_width=1.0)
        ...
        r2.font.name = "Arial"
        r2.font.size = Pt(7.5)
        r2.font.color.rgb = CLR_WHITE

    # Right Box: Competitive Differentiation Matrix (top: 1.95, width: 4.65, height: 4.25)
    _, tf_diff = create_card(slide2, Inches(8.05), Inches(1.95), Inches(4.68), Inches(4.25),
                             bg_color=CLR_WHITE, border_color=CLR_PRIMARY, border_width=1.0)
    ...
    p_dfh.font.color.rgb = CLR_PRIMARY
    ...
    for idx, (c1, c2, c3) in enumerate(comp_rows):
        ...
        r1.font.size = Pt(8)
        r1.font.color.rgb = CLR_PRIMARY if idx == 0 else CLR_TEXT_DARK
        ...
        r3.font.bold = True
        r3.font.size = Pt(8)
        r3.font.color.rgb = CLR_EMERALD if "[✓]" in c3 else CLR_TEXT_DARK

    # Bottom Innovation Strip (top: 6.25, height: 0.45)
    _, tf_btm2 = create_card(slide2, Inches(0.6), Inches(6.25), Inches(12.13), Inches(0.45),
                             bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
    ...
    setup_slide_header(slide3, "TECHNICAL APPROACH: End-to-End Architecture",
                       [("TECHNOLOGIES TO BE USED", CLR_CYAN),
                        ("METHODOLOGY & PROCESS", CLR_PRIMARY),
                        ("SYSTEM FLOWCHARTS", CLR_PRIMARY),
                        ("PROTOTYPE SIMULATION", CLR_EMERALD)])

    # Top Architecture Box (top: 1.38, height: 3.25)
    _, tf_a_bg = create_card(slide3, Inches(0.6), Inches(1.38), Inches(12.13), Inches(3.25),
                             bg_color=CLR_BG_DARK, border_color=CLR_BORDER_BLUE, border_width=1.0)

    # 6 Ingestion Nodes across top of architecture box (top: 1.48)
    sources = [
    ...
    for idx, (s_name, s_sub) in enumerate(sources):
        _, tf_s = create_card(slide3, Inches(0.72 + idx * 1.96), Inches(1.48), Inches(1.88), Inches(0.62),
                              bg_color=CLR_BG_NAVY_CARD, border_color=CLR_CYAN, border_width=1.0)
        p = tf_s.paragraphs[0]
        p.text = f"[FEED] {s_name}"
        p.font.name = "Arial"
        p.font.bold = True
        p.font.size = Pt(8)
        p.font.color.rgb = CLR_CYAN
    ...
    # Normalization Funnel Bar (top: 2.18, height: 0.42)
    _, tf_fn = create_card(slide3, Inches(0.72), Inches(2.18), Inches(11.89), Inches(0.42),
                           bg_color=CLR_SECONDARY, border_color=CLR_CYAN, border_width=1.0)
    ...
    p_fn.font.color.rgb = CLR_WHITE
    p_fn.alignment = PP_ALIGN.CENTER

    # Tripartite Engines (top: 2.68, height: 1.25)
    engines = [
    ...
    for idx, (e_title, e_desc, e_col) in enumerate(engines):
        _, tf_e = create_card(slide3, Inches(0.72 + idx * 4.02), Inches(2.68), Inches(3.85), Inches(1.22),
                              bg_color=CLR_BG_NAVY_CARD, border_color=e_col, border_width=1.0)
        p = tf_e.paragraphs[0]
        p.text = e_title
        p.font.name = "Arial"
        p.font.bold = True
    ...
    # Hard Safety Circuit Breaker Banner (top: 3.98, height: 0.52)
    _, tf_scb = create_card(slide3, Inches(0.72), Inches(3.98), Inches(11.89), Inches(0.52),
                            bg_color=CLR_BG_NAVY_CARD, border_color=CLR_EMERALD, border_width=1.0)
    ...
    # Bottom Left: Live Prototype Simulation (top: 4.72, width: 5.9, height: 1.98)
    _, tf_dm = create_card(slide3, Inches(0.6), Inches(4.72), Inches(5.9), Inches(1.98),
                           bg_color=CLR_BG_BLUE, border_color=CLR_PRIMARY, border_width=1.0)
    ...
    p_dm_h.font.color.rgb = CLR_PRIMARY
    ...
    # Bottom Right: Production Tech Stack (top: 4.72, width: 6.08, height: 1.98)
    _, tf_tc = create_card(slide3, Inches(6.65), Inches(4.72), Inches(6.08), Inches(1.98),
                           bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
    ...
    p_tc_h.font.color.rgb = CLR_CYAN
    ...
    setup_slide_header(slide4, "FEASIBILITY AND VIABILITY: Build, Mitigate & Deploy",
                       [("FEASIBILITY ANALYSIS", CLR_CYAN),
                        ("POTENTIAL RISKS & CHALLENGES", CLR_AMBER),
                        ("STRATEGIES FOR OVERCOMING", CLR_EMERALD),
                        ("DEPLOYMENT & ECONOMICS", CLR_PRIMARY)])

    # Column 1: Implementation Roadmap (top: 1.38, width: 3.0, height: 5.32)
    _, tf_rd = create_card(slide4, Inches(0.6), Inches(1.38), Inches(3.0), Inches(5.32),
                           bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
    ...
    # Column 2: Challenge ➔ Mitigation (top: 1.38, width: 5.15, height: 5.32)
    _, tf_ch = create_card(slide4, Inches(3.75), Inches(1.38), Inches(5.15), Inches(5.32),
                           bg_color=CLR_BG_BLUE, border_color=CLR_PRIMARY, border_width=1.0)
    ...
    p_chh.font.color.rgb = CLR_PRIMARY
    ...
    # Column 3 Top: Deployment Architecture (top: 1.38, width: 3.68, height: 2.60)
    _, tf_dp = create_card(slide4, Inches(9.05), Inches(1.38), Inches(3.68), Inches(2.60),
                           bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
    ...
    # Column 3 Bottom: Deployment Economics & BOM (top: 4.10, width: 3.68, height: 2.60)
    _, tf_cs = create_card(slide4, Inches(9.05), Inches(4.10), Inches(3.68), Inches(2.60),
                           bg_color=CLR_BG_NAVY_CARD, border_color=CLR_EMERALD, border_width=1.0)
    ...
    for cn, cv in cost_items:
        ...
        r2.font.bold = True
        r2.font.size = Pt(7.5)
        r2.font.color.rgb = CLR_EMERALD if "Total" in cn or "₹0" in cv else CLR_CYAN
    ...
    setup_slide_header(slide5, "IMPACT AND BENEFITS: Empowering India's Blue Economy",
                       [("TARGET AUDIENCE IMPACT", CLR_CYAN),
                        ("SOCIAL & HUMAN BENEFITS", CLR_EMERALD),
                        ("ECONOMIC & OPERATIONAL ROI", CLR_AMBER),
                        ("NATIONAL STRATEGIC SCALE", CLR_PRIMARY)])

    # Left Container: ORCA Impact Ecosystem (top: 1.38, width: 5.5, height: 4.75)
    _, tf_ec = create_card(slide5, Inches(0.6), Inches(1.38), Inches(5.5), Inches(4.75),
                           bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
    ...
    # Right Container: 4 Pillars of National Impact (top: 1.38, width: 6.48, height: 4.75)
    _, tf_pl = create_card(slide5, Inches(6.25), Inches(1.38), Inches(6.48), Inches(4.75),
                           bg_color=CLR_BG_BLUE, border_color=CLR_PRIMARY, border_width=1.0)
    ...
    p_plh.font.color.rgb = CLR_PRIMARY
    ...
    # Bottom Progression Banner (top: 6.25, height: 0.45)
    _, tf_btm5 = create_card(slide5, Inches(0.6), Inches(6.25), Inches(12.13), Inches(0.45),
                             bg_color=CLR_BG_DARK, border_color=CLR_CYAN, border_width=1.0)
    ...
    setup_slide_header(slide6, "RESEARCH  AND REFERENCES: Science to Implementation",
                       [("DETAILS OF RESEARCH WORK", CLR_CYAN),
                        ("IMO STABILITY CRITERIA", CLR_PRIMARY),
                        ("HYDRODYNAMIC WAVE PHYSICS", CLR_PRIMARY),
                        ("VERIFIED ACADEMIC CITATIONS", CLR_EMERALD)])

    # 4 Core Research Cards (2x2 Grid: top: 1.38 & 3.75, width: 5.95, height: 2.25)
    research_cards = [
    ...
    for idx, (title, src, insight, app, bcol) in enumerate(research_cards):
        ...
        _, tf_rc = create_card(slide6, x, y, Inches(5.95), Inches(2.28),
                               bg_color=CLR_BG_DARK, border_color=bcol, border_width=1.0)
        p1 = tf_rc.paragraphs[0]
        p1.text = f"📚 {title}"
        p1.font.name = "Arial"
        p1.font.bold = True
        p1.font.size = Pt(9)
        p1.font.color.rgb = CLR_CYAN
    ...
    # Bottom Citations Container (top: 6.20, height: 0.52)
    _, tf_rf = create_card(slide6, Inches(0.6), Inches(6.20), Inches(12.13), Inches(0.52),
                           bg_color=CLR_BG_BLUE, border_color=CLR_PRIMARY, border_width=1.0)
    ...
    p_rf.font.bold = True
    p_rf.font.size = Pt(8)
    p_rf.font.color.rgb = CLR_PRIMARY
    ...
    p_cits.font.color.rgb = CLR_TEXT_DARK

    prs.save(OUTPUT_PATH)
    print(f"\n[SUCCESS] Native Perfect SIH 2026 Presentation generated successfully!")
