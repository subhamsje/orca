...
OUTPUT_PATH = "/Users/subham/Downloads/ORCA_4.0_SIH2026_Epic_10_Slide_Deck.pptx"
...
C_SLATE_100    = RGBColor(241, 245, 249)   # #f1f5f9 - Card background
C_SLATE_200    = RGBColor(226, 232, 240)   # #e2e8f0 - Crisp border
C_SLATE_700    = RGBColor(51, 65, 85)      # #334155 - Body typography
C_SLATE_900    = RGBColor(15, 23, 42)      # #0f172a - Bold headings
...
def add_slide_header(slide, slide_num, total_slides, section_badge, title_text, subtitle_text=""):
    # Top background ribbon
    top_bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(13.333), Inches(1.15))
    set_shape_flat(top_bar, C_WHITE, C_SLATE_200, 0.5)

    # Left: Slide Number & Section Tag
    tx_box = slide.shapes.add_textbox(Inches(0.6), Inches(0.12), Inches(3.0), Inches(0.35))
    tf_tag = tx_box.text_frame
    tf_tag.margin_left = tf_tag.margin_right = tf_tag.margin_top = tf_tag.margin_bottom = 0
    p_tag = tf_tag.paragraphs[0]
    p_tag.text = f"{slide_num:02d}/{total_slides:02d}  ·  {section_badge.upper()}"
    p_tag.font.name = "Arial"
    p_tag.font.size = Pt(8.5)
    p_tag.font.bold = True
    p_tag.font.color.rgb = C_BLUE_PRIMARY

    # Title & Subtitle in Header
    t_box = slide.shapes.add_textbox(Inches(0.6), Inches(0.38), Inches(9.2), Inches(0.70))
    ...
    # Right: ORCA 4.0 Brand Badge
    b_shape, b_tf = create_card(slide, Inches(10.8), Inches(0.18), Inches(1.9), Inches(0.42),
                                bg_color=C_NAVY_HERO, border_color=C_CYAN_ACCENT, border_width=1.0)
    p_b = b_tf.paragraphs[0]
    p_b.text = "ORCA 4.0  ·  SIH2026"
    p_b.font.name = "Arial"
    ...
    # Recurring Decision-Chain Indicator across bottom header
    chain_steps = ["OCEAN", "WEATHER", "VESSEL", "ROUTE", "ORCA", "RISK", "DECISION"]
    active_idx = min(slide_num - 1, len(chain_steps) - 1)
    
    ch_box = slide.shapes.add_textbox(Inches(10.0), Inches(0.68), Inches(2.7), Inches(0.35))
    ...
    for c_i, c_name in enumerate(chain_steps):
        ...
        if c_i < len(chain_steps) - 1:
            ...
            r_arr.font.size = Pt(6.5)
            r_arr.font.color.rgb = C_MUTED

def set_slide_background(slide):
    bg = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(13.333), Inches(7.5))
    set_shape_flat(bg, C_SLATE_50, None)
    return bg
...
def build_presentation():
    ...
    print("Building 10-Slide Epic Investor / Jury Presentation for ORCA 4.0...")
    ...
    print("Generating Slide 1: Hero & Title...")

    # Left: Official SIH Information Form Card
    _, tf_sih = create_card(slide1, Inches(0.6), Inches(0.6), Inches(5.8), Inches(6.3),
                            bg_color=C_WHITE, border_color=C_SLATE_200, border_width=1.0)
    
    p_sih_hdr = tf_sih.paragraphs[0]
    p_sih_hdr.text = "SMART INDIA HACKATHON 2026"
    p_sih_hdr.font.name = "Arial"
    p_sih_hdr.font.size = Pt(16)
    p_sih_hdr.font.bold = True
    p_sih_hdr.font.color.rgb = C_NAVY_HERO
    p_sih_hdr.space_after = Pt(4)

    p_sih_sub = tf_sih.add_paragraph()
    p_sih_sub.text = "NATIONAL FINALS  ·  IDEA PRESENTATION"
    p_sih_sub.font.name = "Arial"
    p_sih_sub.font.size = Pt(9.5)
    p_sih_sub.font.bold = True
    p_sih_sub.font.color.rgb = C_BLUE_PRIMARY
    p_sih_sub.space_after = Pt(16)
    ...
    sih_fields = [
        ...
        ("Ministry / Organization", "ISRO / Department of Space & INCOIS"),
    ...
    # Right: Large Premium ORCA Showcase Panel (Deep Navy)
    _, tf_hero = create_card(slide1, Inches(6.7), Inches(0.6), Inches(6.0), Inches(6.3),
    ...
    p_h_desc.font.size = Pt(10)
    p_h_desc.font.color.rgb = RGBColor(203, 213, 225)
    p_h_desc.space_after = Pt(14)
    ...
    p_dc_flow.font.size = Pt(8.5)
    p_dc_flow.font.bold = True
    p_dc_flow.font.color.rgb = C_WHITE
    p_dc_flow.space_after = Pt(14)

    # 3 Hero Value Pillars
    s1_pillars = [
    ...
    add_slide_header(slide2, 2, 10, "Problem Discovery", 
    ...
    print("Generating Slide 2: The Problem & Decision Gap...")

    # Hero Thesis Callout Box across top
    _, tf_s2_th = create_card(slide2, Inches(0.6), Inches(1.35), Inches(12.13), Inches(0.60),
    ...
    _, tf_s2_c = create_card(slide2, Inches(5.6), Inches(2.10), Inches(3.2), Inches(4.8),
    ...
    p_c_h.font.size = Pt(10)
    p_c_h.font.color.rgb = C_ROSE
    p_c_h.space_after = Pt(8)
    ...
    # Right: The ORCA Paradigm (width: 3.7)
    _, tf_s2_r = create_card(slide2, Inches(9.0), Inches(2.10), Inches(3.73), Inches(4.8),
                             bg_color=C_EMERALD_BG, border_color=C_EMERALD, border_width=1.5)
    p_r_h = tf_s2_r.paragraphs[0]
    p_r_h.text = "ORCA 4.0 DECISION INTELLIGENCE"
    p_r_h.font.name = "Arial"
    p_r_h.font.bold = True
    p_r_h.font.size = Pt(10.5)
    p_r_h.font.color.rgb = C_EMERALD
    p_r_h.space_after = Pt(8)
    ...
    add_slide_header(slide3, 3, 10, "Solution Overview", 
    ...
    _, tf_s3_eng = create_card(slide3, Inches(4.0), Inches(1.35), Inches(5.2), Inches(5.55),
                               bg_color=C_NAVY_HERO, border_color=C_CYAN_ACCENT, border_width=1.5)
    ...
    add_slide_header(slide4, 4, 10, "Competitive Landscape", 
    ...
    # Right: The Contextual Difference Statement Box (width: 4.33)
    _, tf_s4_r = create_card(slide4, Inches(8.4), Inches(1.35), Inches(4.33), Inches(5.55),
    ...
    p_s4_rh.font.size = Pt(10.5)
    p_s4_rh.font.color.rgb = C_BLUE_PRIMARY
    p_s4_rh.space_after = Pt(8)
    ...
    add_slide_header(slide5, 5, 10, "System Architecture", 
    ...
    print("Generating Slide 5: Technical Architecture...")

    # Layer 1: Ingestion Sources (top: 1.35, height: 0.85)
    _, tf_l1 = create_card(slide5, Inches(0.6), Inches(1.35), Inches(12.13), Inches(0.85),
                           bg_color=C_WHITE, border_color=C_SLATE_200, border_width=1.0)
    ...
    # Layer 2: Normalization & Dual Digital State (top: 2.30, height: 0.95)
    _, tf_l2 = create_card(slide5, Inches(0.6), Inches(2.30), Inches(12.13), Inches(0.95),
    ...
    p_l2_b.font.size = Pt(8)
    p_l2_b.font.color.rgb = C_SLATE_700

    # Layer 3: Tripartite Computing Engines (top: 3.35, height: 1.45)
    _, tf_l3 = create_card(slide5, Inches(0.6), Inches(3.35), Inches(12.13), Inches(1.45),
                           bg_color=C_NAVY_HERO, border_color=C_CYAN_ACCENT, border_width=1.0)
    ...
    # Layer 4: Hard Safety Circuit Breaker Shield (top: 4.90, height: 0.85)
    _, tf_l4 = create_card(slide5, Inches(0.6), Inches(4.90), Inches(12.13), Inches(0.85),
                           bg_color=C_EMERALD_BG, border_color=C_EMERALD, border_width=1.5)
    ...
    p_l4_b.font.size = Pt(8)
    p_l4_b.font.color.rgb = C_SLATE_900

    # Layer 5: Operational Deliverables (top: 5.85, height: 0.95)
    _, tf_l5 = create_card(slide5, Inches(0.6), Inches(5.85), Inches(12.13), Inches(0.95),
                           bg_color=C_WHITE, border_color=C_SLATE_200, border_width=1.0)
    ...
    add_slide_header(slide6, 6, 10, "Intelligence Separation", 
    ...
    print("Generating Slide 6: Intelligence Engine...")

    # 3 Tripartite Columns (width: 3.85 each)
    tri_data = [
    ...
    for idx, (t_title, t_sub, t_items, bg_c, b_col) in enumerate(tri_data):
        _, tf_tri = create_card(slide6, Inches(0.6 + idx * 4.14), Inches(1.35), Inches(3.85), Inches(4.35),
    ...
    # Bottom Banner: The Constrained LLM Boundary (top: 5.80, height: 1.10)
    _, tf_llm = create_card(slide6, Inches(0.6), Inches(5.80), Inches(12.13), Inches(1.10),
                            bg_color=C_NAVY_HERO, border_color=C_CYAN_ACCENT, border_width=1.0)
    p_llm_h = tf_llm.paragraphs[0]
    p_llm_h.text = "THE LLM SAFETY GOVERNANCE DIRECTIVE"
    p_llm_h.font.name = "Arial"
    p_llm_h.font.bold = True
    p_llm_h.font.size = Pt(9.5)
    ...
    add_slide_header(slide7, 7, 10, "Prototype Validation", 
    ...
    # Right Top: Map Navigation Simulation Diagram (width: 4.73)
    _, tf_s7_map = create_card(slide7, Inches(8.0), Inches(1.35), Inches(4.73), Inches(3.20),
                               bg_color=C_BLUE_LIGHT, border_color=C_BLUE_BORDER, border_width=1.0)
    ...
    p_map_e.font.size = Pt(8.5)
    p_map_e.font.color.rgb = C_EMERALD

    # Right Bottom: Live Telemetry Metric Badges (width: 4.73)
    _, tf_s7_met = create_card(slide7, Inches(8.0), Inches(4.70), Inches(4.73), Inches(2.20),
                               bg_color=C_SLATE_50, border_color=C_SLATE_200, border_width=1.0)
    p_s7_th = tf_s7_met.paragraphs[0]
    p_s7_th.text = "BENCHMARK PERFORMANCE (SIMULATION)"
    p_s7_th.font.name = "Arial"
    p_s7_th.font.bold = True
    p_s7_th.font.size = Pt(9.5)
    p_s7_th.font.color.rgb = C_NAVY_HERO
    p_s7_th.space_after = Pt(3)
    ...
    add_slide_header(slide8, 8, 10, "Resilience & Failsafes", 
    ...
    _, tf_s8_r = create_card(slide8, Inches(6.4), Inches(1.35), Inches(6.33), Inches(5.55),
                             bg_color=C_EMERALD_BG, border_color=C_EMERALD, border_width=1.5)
    ...
    p_s8_rh.font.size = Pt(10.5)
    p_s8_rh.font.color.rgb = C_EMERALD
    p_s8_rh.space_after = Pt(6)
    ...
    add_slide_header(slide9, 9, 10, "Execution & Scale", 
    ...
    print("Generating Slide 9: Feasibility, Deployment & Economics...")

    # Zone 1: Verified Roadmap (width: 3.6)
    _, tf_s9_z1 = create_card(slide9, Inches(0.6), Inches(1.35), Inches(3.6), Inches(5.55),
                              bg_color=C_WHITE, border_color=C_SLATE_200, border_width=1.0)
    p_z1_h = tf_s9_z1.paragraphs[0]
    p_z1_h.text = "ZONE 1: VERIFIED ROADMAP"
    p_z1_h.font.name = "Arial"
    p_z1_h.font.bold = True
    p_z1_h.font.size = Pt(10)
    p_z1_h.font.color.rgb = C_NAVY_HERO
    p_z1_h.space_after = Pt(6)
    ...
    # Zone 2: Mesh Deployment Architecture (width: 4.2)
    _, tf_s9_z2 = create_card(slide9, Inches(4.4), Inches(1.35), Inches(4.2), Inches(5.55),
                              bg_color=C_SLATE_50, border_color=C_SLATE_200, border_width=1.0)
    p_z2_h = tf_s9_z2.paragraphs[0]
    p_z2_h.text = "ZONE 2: DEPLOYMENT ARCHITECTURE"
    p_z2_h.font.name = "Arial"
    p_z2_h.font.bold = True
    p_z2_h.font.size = Pt(10)
    ...
    # Zone 3: BOM Economics & Scaling (width: 3.93)
    _, tf_s9_z3 = create_card(slide9, Inches(8.8), Inches(1.35), Inches(3.93), Inches(5.55),
    ...
    p_z3_h.font.size = Pt(10)
    p_z3_h.font.color.rgb = C_EMERALD
    p_z3_h.space_after = Pt(6)
    ...
    add_slide_header(slide10, 10, 10, "Impact & Vision", 
    ...
    print("Generating Slide 10: Impact, Research & Final Vision...")
    ...
    for idx, (rt, rsrc, rapp, bg_c, b_col) in enumerate(research_s10):
        _, tf_r = create_card(slide10, Inches(0.6 + idx * 3.08), Inches(1.35), Inches(2.9), Inches(1.85),
                              bg_color=bg_c, border_color=b_col, border_width=1.0)
    ...
    # Center: 5-Stakeholder Multiplier Ring (top: 3.32, height: 1.65)
    _, tf_stk = create_card(slide10, Inches(0.6), Inches(3.32), Inches(12.13), Inches(1.65),
                            bg_color=C_WHITE, border_color=C_SLATE_200, border_width=1.0)
    ...
    # Bottom: Final Heroic Vision Statement (Deep Navy: top: 5.08, height: 1.82)
    _, tf_vis = create_card(slide10, Inches(0.6), Inches(5.08), Inches(12.13), Inches(1.82),
                            bg_color=C_NAVY_HERO, border_color=C_CYAN_ACCENT, border_width=1.5)
